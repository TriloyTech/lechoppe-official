...
export async function workOne(pool:Pool, send = sendReservationEmail) {
 ...
 WHERE id=(SELECT id FROM reservation_notifications WHERE (status='pending' AND next_attempt_at<=now()) OR (status='processing' AND lease_until<now()) ORDER BY created_at FOR UPDATE SKIP LOCKED LIMIT 1) RETURNING id,event_id`,[token,process.env.RESEND_FROM_EMAIL || null]);
 ...
 await transaction(pool,async client=>{
   ...
   if (!job.payload.from) job.payload.from=process.env.RESEND_FROM_EMAIL;
   try {
     ...
     const error = e instanceof Error && /^(provider_(missing|uncertain|http_\d{3}))$/.test(e.message) ? e.message : 'provider_uncertain';
