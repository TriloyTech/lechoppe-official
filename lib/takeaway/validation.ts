...
export function parseOrderPayload(input: unknown): CreateOrderPayload {
  ...
  for (const line of body.items) { if (!UUID.test(line.item_id) || !Number.isInteger(line.quantity) || line.quantity < 1 || line.quantity > 100 || !Array.isArray(line.choice_ids) || line.choice_ids.length > 30 || new Set(line.choice_ids).size !== line.choice_ids.length || line.choice_ids.some((id) => !UUID.test(id)) || String(line.special_instructions ?? "").length > 300) throw new Error("Invalid order line"); }
