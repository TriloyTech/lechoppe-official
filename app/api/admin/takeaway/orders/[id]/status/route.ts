...
const TRANSITIONS: Record<OrderStatus, OrderStatus[]> = { NEW: ["ACCEPTED", "CANCELLED"], ACCEPTED: ["PREPARING", "CANCELLED"], PREPARING: ["READY", "CANCELLED"], READY: ["COMPLETED", "NO_SHOW"], COMPLETED: [], CANCELLED: [], NO_SHOW: [] };
...
export async function POST(request: NextRequest, context: Context) {
  ...
  const { id } = await context.params; const body = await request.json(); const next = String(body.status) as OrderStatus; const reason = String(body.reason_code ?? ""); const note = String(body.note ?? "").trim().slice(0, 500);
  if (!(next in TRANSITIONS)) return NextResponse.json({ error: "Invalid status" }, { status: 400 });
