...
import { canChangePayment } from "@/lib/takeaway/lifecycle";
...
export async function POST(request: NextRequest, context: Context) {
  ...
  const status = String(body.payment_status); const method = body.payment_method as PaymentMethod | undefined;
  ...
  try {
    ...
    if (!canChangePayment(current.rows[0].status, status)) { await client.query("ROLLBACK"); return NextResponse.json({ error: "Completed orders must remain paid" }, { status: 409 }); }
    const updated = await client.query("UPDATE takeaway_orders SET payment_status=$1,payment_method=$2,paid_at=$3 WHERE id=$4 RETURNING id,status,payment_status,payment_method,paid_at", [status, status === "PAID" ? method : null, status === "PAID" ? new Date() : null, id]);
