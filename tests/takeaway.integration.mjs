...
try {
  ...
  const soldOutItem = await database.query("INSERT INTO menu_items(name,price,category,available,takeaway_available,vat_rate) VALUES ('Sold out item',11,'burger',false,true,10) RETURNING id");
  await database.query("INSERT INTO menu_items(name,price,category,available,takeaway_available,vat_rate) VALUES ('Dine in only',12,'burger',true,false,10),('Missing VAT',13,'burger',true,false,NULL),('Unclassified',14,'not_configured',true,true,10)");
  ...
  const menu = await request("/api/takeaway/menu"); assert.equal(menu.response.status, 200); assert.equal(menu.body.items.find((entry) => entry.id === item.rows[0].id).available, true); assert.equal(menu.body.items.find((entry) => entry.id === soldOutItem.rows[0].id).available, false); assert.equal(menu.body.items.some((entry) => entry.name === "Dine in only"), false); assert.equal(menu.body.items.some((entry) => entry.name === "Missing VAT"), false); assert.equal(menu.body.items.some((entry) => entry.name === "Unclassified"), false);
