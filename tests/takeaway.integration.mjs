...
try {
  ...
  const payload = async (suffix, pickupTime = first.value, promoCode, pickupType = "asap") => ({ customer_name: `Test ${suffix}`, customer_email: `test-${suffix}@example.com`, customer_phone: suffix === "a" ? "06 12 34 56 78" : "06-87-65-43-21", pickup_time_type: pickupType, pickup_time: pickupTime, lang: "fr", website: "", ...(promoCode ? { promo_code: promoCode } : {}), items: [{ item_id: item.rows[0].id, choice_ids: [], special_instructions: "Sauce à part", quantity: 1 }], ...(await challenge()) });
  ...
  console.log("integration PASS: real Next endpoints, auth/settings/offers, promo eligibility, concurrent order capacity, ASAP, tracking, cancellation, lifecycle, payment, migration reruns");
} catch (error) {
