...
function workerSendDb(overrides={},current={kind:'received',status:'pending'},ending=[]){return scripted([[/SKIP LOCKED/,[{id:'job',event_id:'event'}]],[/BEGIN/],[/FOR UPDATE OF r/,[current]],[/claim_token=.*FOR UPDATE/,[{id:'job',audience:'customer',recipient:'a@example.com',attempts:1,first_attempt_at:new Date(),payload:{message:renderReservationTest(),from:'sender@example.com'},...overrides}]],[/COMMIT/],[/BEGIN/],[/claim_token=.*FOR UPDATE/,[{id:'job'}]],[/UPDATE reservation_notifications SET status/],...ending,[/COMMIT/]]);}
test('worker safe claim, immutable provider key, accepted ID, recoverable lease and bounded backoff',async()=>{
 env();const db=workerSendDb();let args;await workOne(db.pool,async(...a)=>{args=a;return 'provider-id';});db.done();assert.equal(args[2],'reservation/job');const claim=db.calls[0].sql;assert.match(claim,/lease_until<now\(\)/);assert.match(claim,/attempts=attempts\+1/);assert.equal(db.calls.find(c=>c.sql.startsWith('UPDATE reservation_notifications SET status=')&&c.args[1]==='accepted').args[2],'provider-id');assert.equal(backoffSeconds(1),30);assert.equal(backoffSeconds(20),3600);
});
test('worker failure persists sanitized retry; final attempt fails; stale and expired jobs never send',async()=>{
 env();for(const attempts of [1,6]){const db=workerSendDb({attempts},undefined,[[/next_attempt_at/]]);await workOne(db.pool,async()=>{throw new Error('private secret');});db.done();assert.equal(db.calls.find(c=>c.args[2]==='provider_uncertain').args[1],attempts===6?'failed':'pending');}
