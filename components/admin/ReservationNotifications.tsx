...
export default function ReservationNotificationSettings(){
 ...
 const problemLabel=(code:string)=> {
  ...
  if(code==='provider_missing')return t({fr:'Configuration du fournisseur d’e-mail manquante.',en:'Email provider configuration missing.',es:'Falta la configuración del proveedor de correo.',it:'Configurazione del provider email mancante.'});
  if(code==='provider_auth_failed')return t({fr:'Échec de l’authentification.',en:'Authentication failed.',es:'Error de autenticación.',it:'Autenticazione non riuscita.'});
  if(code==='provider_timeout')return t({fr:'Délai de connexion dépassé.',en:'Connection timeout.',es:'Tiempo de espera de conexión agotado.',it:'Timeout di connessione.'});
  if(code==='provider_rejected')return t({fr:'Message rejeté.',en:'Message rejected.',es:'Mensaje rechazado.',it:'Messaggio rifiutato.'});
  if(code==='provider_uncertain')return t({fr:'Livraison incertaine.',en:'Delivery uncertain.',es:'Entrega incierta.',it:'Consegna incerta.'});
  ...
  return t({fr:'Livraison incertaine.',en:'Delivery uncertain.',es:'Entrega incierta.',it:'Consegna incerta.'});
 };
 ...
 invalid:t({fr:'Adresse e-mail invalide. Vérifiez le format puis réessayez.',en:'Invalid email address. Check the format and retry.',es:'Dirección de correo no válida. Revise el formato y reintente.',it:'Indirizzo email non valido. Controlla il formato e riprova.'}),
 ...
 forbidden:t({fr:'Action refusée pour cette origine. Rechargez la page et réessayez.',en:'Action refused for this origin. Reload the page and retry.',es:'Acción rechazada para este origen. Recargue la página y reintente.',it:'Azione rifiutata per questa origine. Ricarica la pagina e riprova.'}),
 ...
 {data && <>
 {!data.providerConfigured && <p role="alert">{t({fr:'Configuration du fournisseur d’e-mail manquante.',en:'Email provider configuration missing.',es:'Falta la configuración del proveedor de correo.',it:'Configurazione del provider email mancante.'})}</p>}
